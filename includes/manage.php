...
class Manage  
{
	...
	public function manageRecordWithPagination($table,$pno){
		...
		if ($table == "categories") {
			# code...
			$sql = "SELECT p.category_name as category, c.category_name as parent, p.status, p.cid FROM categories p LEFT JOIN categories c ON p.parent_cat=c.cid ".$a["limit"];
		...
		public function deleteRecord($table, $pk, $id){
			if ($table == "categories") {
				...
				if ($result->num_rows > 0) {
					...
					$pre_stmt = $this->con->prepare("DELETE FROM ".$table." WHERE ".$pk." = ? LIMIT 1");
					$pre_stmt->bind_param("i", $id);
				    $result = $pre_stmt->execute() or die($this->con->error);
			...
			if ($invoice_no != null) {
				...
				for ($i=0; $i <count($ar_price) ; $i++) { 
					...
					if ($rem_qty < 0) {
						...
						$this->con->query("UPDATE products SET product_stock = '$rem_qty' WHERE product_name = '".$ar_pro_name[$i]."'");
					}
					//END
